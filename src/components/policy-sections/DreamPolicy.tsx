...
export function DreamPolicy({ data, onChange }: Props) {
  const update = (partial: Partial<PolicyData>) => onChange({ ...data, ...partial })

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <Label className="text-base font-semibold">Enable Dream Policy</Label>
        <Switch
      ...
      <div className={`grid gap-4 md:grid-cols-2 ${!data.enabled ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div className="space-y-2">
          <Label>Max Dream Attempts</Label>
          <Input
            type="number"
            value={data.max_dream_attempts ?? ''}
            onChange={(e) => update({ max_dream_attempts: e.target.value ? Number(e.target.value) : null })}
          />
        </div>

        <div className="space-y-2">
          <Label>Max Dream Offers</Label>
          <Input
            type="number"
            value={data.max_dream_offers ?? ''}
            onChange={(e) => update({ max_dream_offers: e.target.value ? Number(e.target.value) : null })}
          />
        </div>
